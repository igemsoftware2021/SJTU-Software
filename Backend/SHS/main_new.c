...
double DHstr = 2; // score for domain ending in a base pair  解决interface_problem
...
//    for(i=0;i<num_domain;i++) printf("%s\n",char_domain[i]);
    ...
    for(i=0;i<num_domain;i++) free(char_domain[i]);
    free(char_domain);

    for(i=0;i<num_domain;i++) free(domain[i]);
    free(domain);

    free(domain_length);

    free(domain_gatc_avail);

    free(domain_importance);

    free(domain_score);

    free(domain_intrinsic);

    for (i = 0; i < 1; i++) free(crosstalk[i]);
    free(crosstalk);

    for( i = 0; i < 1; i++) free(interaction[i]);
    free(interaction);
