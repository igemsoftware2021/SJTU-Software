...
def featSelect_plot(data, y):
    # 选三个指标进行作图
    miRNA_names, miRNA_names_up, miRNA_names_down = RDA_new_plot(data)
    ...
    for i in range(len(miRNA_names)):
        for j in range(i+1, len(miRNA_names)):
            for k in range(j+1, len(miRNA_names)):
                X = data_.iloc[[i, j, k], :]
                X = np.array(X)
                X = np.transpose(X)
