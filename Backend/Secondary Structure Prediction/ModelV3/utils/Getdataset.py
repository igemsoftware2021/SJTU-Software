...
class IntelDNADataset(Dataset):
    def __init__(self, data_dir, adj_dir, length, wv_size, data_type='train'):
    ...
    def __getitem__(self, idx):
        ...
        with open(self.bp_name) as f:
            ...
            model = word2vec.Word2Vec(List, window=8, size=self.encode_size, min_count=0)
            ...
            seq = seq + '#' * (self.length - lengths)

            zero = np.zeros(self.encode_size, dtype=np.float32)
            self.onehot = defaultdict(lambda: np.ones(self.encode_size, dtype=np.float32) / self.encode_size,
        ...
        # return seq, encode_seq, result, lengths
